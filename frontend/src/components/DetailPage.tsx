    ...
    return (
        <div className="max-w-5xl mx-auto p-4 sm:p-6 md:p-8">
            <header className="mb-8 flex items-start gap-6">
                <button onClick={onBack} className="flex-shrink-0 bg-gray-700 hover:bg-gray-600 text-white font-bold py-2 px-4 rounded-md transition-colors duration-300 flex items-center mt-1">
                    <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
                    返回列表
                </button>
                <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-white text-wrap">{analysis.data.article_title || 'Untitled Analysis'}</h1>
            </header>

            <div className="bg-gray-800 p-6 md:p-8 rounded-2xl shadow-2xl">
                <div className="mb-8 p-5 bg-gray-900/50 rounded-xl text-gray-300 italic border border-gray-700">
                    <h3 className="font-bold text-lg text-white not-italic mb-2">AI 總結</h3>
                    {analysis.data.analysis_summary}
                </div>
                <h2 className="text-xl font-bold text-white mb-4">校對文章 (滑鼠懸停查看錯誤分析)</h2>
                <div className="bg-gray-900/50 p-6 md:p-8 rounded-xl text-gray-200 text-lg leading-loose border border-gray-700">
                    {renderAnnotatedPost()}
                </div>
            </div>
        </div>
